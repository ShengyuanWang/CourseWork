...
getArguments(argc, argv, &nx, &ny, &steps, &display);

float  u[2][nx][ny];        /* 3D array for grid */
                           // see https://iq.opengenus.org/3d-array-in-c/ 
...
if (taskid != CONDUCTOR)
{
  ...
  MPI_Barrier(MPI_COMM_WORLD);   // LS workers each wait until conductor done intializing
