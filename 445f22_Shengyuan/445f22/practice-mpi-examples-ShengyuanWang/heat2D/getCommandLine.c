...
void getArguments(int argc, char *argv[], int * nx, int * ny, int * steps, int * display)
{
  ...
  int display_flag = 0; 
  // for verbose printing output (not yet implemented)
  // int verbose_flag = 0;


  while ((c = getopt (argc, argv, "x:y:s:d")) != -1) {
...
void Usage(char *program) {
  ...
  fprintf(stderr, "Usage: %s [-x rows] [-y cols] [-s steps] [-d]\n", program);
}
