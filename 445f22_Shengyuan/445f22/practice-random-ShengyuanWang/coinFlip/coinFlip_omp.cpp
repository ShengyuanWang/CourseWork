...
//   random number generator. When  -c used, the seed will be constant,
//      otherwise the seed is based on the clock time of the machine.
...
// fork threads to generate random numbers in parallel
  ...
  reduction(+:numHeads, numTails)
  {
    ...
    if (numThreads > 1) {
      ...
      rand.jump(tid * (trialFlips / numThreads));  // this is the leapfrog setup
    }
    // each thread will get trialFlips/numThreads total random values
