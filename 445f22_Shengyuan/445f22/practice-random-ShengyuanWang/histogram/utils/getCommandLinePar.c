...
// void getDoubleArguments(int argc, char *argv[], int * numThreads, int * N, \
...
void getArguments(int argc, char *argv[], int * numThreads, int * N, int * numBins, int * min, int * max, int * print, int * useConstantSeed)
{
  char *tvalue;
  char *nvalue;
  char *bins_value;
  ...
  while ((c = getopt (argc, argv, "pt:n:i:a:b:chv")) != -1) {
    switch (c)
      {
      ...
      case 'a':
        min_value = optarg;
        // *min = strtod(min_value, NULL);
        *min = atoi(min_value);
        break;

      case 'b':
        max_value = optarg;
        // *max = strtod(max_value, NULL);
        *max = atoi(max_value);
        break;
