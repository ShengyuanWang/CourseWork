/* conductorWorker.c
 * ... illustrates the conductor-worker pattern in OpenMP
 *
 * Joel Adams, Calvin College, November 2009.
 *
 * Usage: ./conductorWorker
 *
 * Exercise: 
   ...
   #pragma omp parallel 
    {
        ...
        if ( id == 0 ) {  // thread with ID 0 is conductor
            printf("Greetings from the conductor, # %d of %d threads\n",
			    id, numThreads);
