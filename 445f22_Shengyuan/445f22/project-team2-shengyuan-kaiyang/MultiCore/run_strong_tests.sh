...
###
 ...
 # @Date: 2022-11-21 20:32:49
 # @LastEditors: Shengyuan Wang
 # @LastEditTime: 2022-12-08 15:24:13
 # @FilePath: /445f22/project-team2-shengyuan-kaiyang/MultiCore/run_strong_tests.sh
 # @Description: Strong Scalability Test for OpenACC-Multicore
 # 
### 
num_times=$1
    ...
    do
        ...
        do
            if [ "$num_threads" == "1" ]; then
                command="./floyd_mc -n $problem_size -t 1 -e"
            else
                command="./floyd_mc -n $problem_size -t $num_processes -e"
            fi
            $command
        done
        printf "\n"
