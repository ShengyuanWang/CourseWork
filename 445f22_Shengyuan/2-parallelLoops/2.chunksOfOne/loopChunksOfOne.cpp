...
int main(int argc, char* argv[]) {
  ...
  int min = 1;
  int max = 99;
  // for method the random generator with use to dole out the numbers
  ...
  getArguments(argc, argv, &numThreads, &repetitions, &useConstantSeed, &doleOut);
...
//  -the loop counter 
//
//  Since seedValue and repetitions are read but not written, they are shared.
//
...
shared(seedValue,repetitions, min, max, numThreads, doleOut)
{
  ...
  // Note here a different loop structure- programmer decides which
  // parts of the loop a thread will complete.
  int i;
 // no openMP pragma for this version
  for (i=tid; i < repetitions; i+=numThreads) {
